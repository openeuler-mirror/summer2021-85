...
void send_sig_info_to_user(struct ksignal *ksig);
...
relock:
	...
	for (;;) {
		...
		if (sig_kernel_stop(signr)) {
			...
			// signal to stop all thread should be monitored
			prepare_for_nt_send();
			
			// send info and print in kernel
			// the place can be moved?
			send_sig_info_to_user(ksig);
...
/*
 * Zeyu signal monitor test fragment
 */
...
int netlink_sig_init(void)
{
    ...
    if(sig_monitor_sk == NULL){
        printk(KERN_ERR "netlink_sig: create netlink socket error.\n");
        return 1;
    }
    printk("netlink_sig: create netlink socket ok.\n");
    return 0;
}
...
void send_sig_info_to_user(struct ksignal *ksig) {
    if(ksig->info.si_code == SI_USER || ksig->info.si_code == SI_KERNEL){
		int sender = ksig->info._sifields._kill._pid;
		sprintf(sig_send_buf, "ZZZeyu shows: sender(%d) send a signal! \n", sender);
		send_nlmsg(sig_send_buf, strlen(sig_send_buf), SIG_RECV_PID);
		printk(KERN_INFO "ZZZeyu shows: sender(%d) send a signal! \n", sender);
	}
	sprintf(sig_send_buf, "ZZZeyu shows: process id: %d got a signal signo is %d \n", current->pid, ksig->sig);
	send_nlmsg(sig_send_buf,  strlen(sig_send_buf), SIG_RECV_PID);
	printk(KERN_INFO "ZZZeyu shows: process id: %d got a signal signo is %d \n", current->pid, ksig->sig);
}

/*
 * recv_callback - This function only hold the space currently
 */
void recv_callback(struct sk_buff *skb)
{
